...
export default function CountrySelect({ formData, onChange }) {
  ...
  useEffect(() => {
    let unmounted = false;
    window?.fetch("/api/swell/get-shipment-zones", {}).then(async (res) => {
      if (res.status === 200 && !unmounted) {
        setAvailableCountries(await res.json());
      }
    });
  ...
  return (
    <Autocomplete
      ...
      onChange={(event, value) => {
        onChange((x) => x.setIn(["country"], value?.code || ""));
      }}
      options={countries.filter((a) => {
        return availableCountries.includes(a.code);
      })}
      renderInput={(params) => (
        <TextField
          {...params}
          error={Boolean(invalidCountry)}
          helperText={invalidCountry}
          inputProps={{
            ...params.inputProps,
            autoComplete: "new-password", // disable autocomplete and autofill
          }}
          label={t("Choose a country", { ns: "Checkout" })}
          required
        />
      )}
...
const countries = [
  ...
  { code: "CZ", label: "Czech Republic", suggested: true },
  {
    code: "DE",
    label: "Germany",
  },
