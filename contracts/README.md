...
1. `nvm use` in project root
2. install truffle-cli `npm install -g truffle`
3. `touch ./contracts/.env` with 

    ```
    NODE_OPTIONS="--openssl-legacy-provider"
    MNEMONIC="<Your Mnemonic>"
    INFURA_PROJECT_ID="e199fad25bdc44f08be31b3b523957ad"
    ```

    Mnemonic from 1password `meetvers-dev-wallet-private-key`. Login to infura in 1password too (login `jan.fabian+infura@topmonks.com`)

4. `truffle migrate --config=truffle-config.polygon.js  --network=polygon_infura_testnet`

Optionally local deployment

5. install `https://www.npmjs.com/package/ganache-cli`

6. `ganache` to start local RPC node

7. `truffle migrate --config=truffle-config.polygon.js  --network=development`

# Local testing

1. `ganache-cli --fork https://polygon-mumbai.infura.io/v3/e199fad25bdc44f08be31b3b523957ad`
2. `truffle migrate --config=truffle-config.js --network=development`
3. `truffle exec scripts/script.js --config=truffle-config.js --network=development`
4. `truffle test --config=truffle-config.js --network=development`
5. `truffle debug --config=truffle-config.js --network=development <transaction-hash> --fetch-external`

# Verify contract

`npx truffle run verify Meetvers --config=truffle-config.polygon.js --network=polygon_infura_mainnet`
